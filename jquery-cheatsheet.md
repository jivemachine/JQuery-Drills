...
$('.demo').outerHeight(); // computed height of first element in set including padding and border
```
...
### Triggering:
```
$('a#mylink').trigger('click'); // triggers click event on an <a> element with id mylink
```
...
### Chaining:
JQuery methods return a reference to the created JQuery object to allow method chaining like the following:
```
$('.demo').css('backgroundColor', 'green').slideUp(500).slideDown(500);
```

# JQuery Documentation
- Jquery API: [here](http://api.jquery.com/)